...
We are going to use the same dataset for the rest of this courses lesson examples (exercises will use different datasets). This dataset is from NWIS and was created using [this R code](https://github.com/USGS-R/gsIntroR/blob/master/R/create_df.R). Download the required csv from [here](). 

```{r read_csv_examp}
# Read in the data and take a look at it
intro_df <- read.csv("inst/extdata/course_NWISdata.csv")
head(intro_df)
str(intro_df)
dim(intro_df)
summary(intro_df)
...
There are a variety of ways to pull data from Excel - we recommend saving it as a csv and reading it in using `read.csv`. In Excel, File >> Save As >> Save as type >> CSV (Comma delimited). 
...
```{r, eval=FALSE}
str(read.csv("inst/extdata/course_NWISdata.csv"))
str(read.csv("inst/extdata/course_NWISdata.csv", stringsAsFactors=FALSE))
```

Another common issue with reading in data frames for USGS data is that site numbers often have a leading zero that is dropped if it defaults to an integer. To prevent this, you can specify the class for each column, using NA for ones that you would like R to choose. In our dataset, we know that site numbers (column one) should be treated as character. We aren't positive about the other six, so we say `NA` in the arguments for `colClasses` to indicate we want `read.table` to choose for us. See `?read.table` for more information about how that how that works.

```{r}
intro_df <- read.csv("inst/extdata/course_NWISdata.csv", stringsAsFactors = FALSE, 
